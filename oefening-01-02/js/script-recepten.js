    ...
    for (let i = 0; i < json.length - 2; i++) {
      ...
      html += `<div class="accordion-item">
            ...
            <h2 class="accordion-header">
              ...
              <img src="${recipe.image}" class="img-small col-md-3 pe-3 float-start" alt="${recipe.title}"><span class="h6 mt-2 koffiebruin">ingrediënten</span>:${recipe.ingredients}<hr><span class="h6  mt-2 koffiebruin">bereiding</span>:${recipe.description}         </div>
            </div>
      </div>`;
    }
    html += '</div>';
